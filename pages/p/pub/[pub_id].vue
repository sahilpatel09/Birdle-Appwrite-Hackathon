...
async function getPubData(){
	const data = await service.getPublication(route.params.pub_id);
	console.log(data)
	docID.value = data.$id
	name.value = data.name
	desc.value = data.subtitle
	writers.value = data.writers
	followCount.value = data.followers_count
	const url = await service.getThumbnail(data.img);
	console.log("URL", url.href)
	pubImageShowURL.value = url.href
...
async function createPub() {
  ...
  const dataObject = {
    ...
    followers_count: followCount.value,
  ...
  const create = await service.updatePub(docID.value, dataObject);
